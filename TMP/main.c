...
void print_frame_refcount()
{
        ...
        for(i = 0;i<1024;i++)
        {
                ...
                {
                        kprintf("               FRAME: %d   REFCOUNT: %d \n",i,frm_tab[i].fr_refcnt);
                }
        }
}
*/
...
void proc_test2(int i,int j,int* ret,int s)
{
  	...
  	int r;
  	bsize = get_bs(i, j);
	//kprintf("PID: %d BSIZE: %d BSID: %d REQUESTED: %d \n",currpid,bsize,i,j);
  	if (bsize != 50)
	{
		//kprintf("1. FAILED\n");
    		*ret = TFAILED;
...
void proc1_test4(int* ret) {
  ...
  /*Shoud see what proc 2 updated*/
	for (i = 0; i < 2; i++) 
	{
    	...
    	if (*(addr + i * NBPG) != 'a'+i)
   	{
		//kprintf("PROC1: Test Failed \n");
      		*ret = TFAILED;
      		break;    
    	}
	else
	{
		//kprintf("PROC1: Test Success \n");
	}
  	}
...
//kprintf("PROC2 ADDR: %ul\n",addr);
  /*Shoud see what proc 1 updated*/
	...
	if (*(addr + i * NBPG) != 'A'+i)
	{
		//kprintf("PROC2: Test Failed \n");
      		*ret = TFAILED;
      		break;
    	}
	else
	{
		//kprintf("PROC2: Test Success \n");
	}
	}
...
void print_accessbit(unsigned long vaddr)
{
	...
	kprintf("\nPD_OFF: %ul PT_OFF: %d PDBR: %ul, PT_BASE: %ul DIR: %ul PD_BASE: %d\n",pd_offset, pt_offset, pdbr, pt_base,dir,dir->pd_base);
...
void print_frame_status()
{
	...
	for(i = 0;i<1024;i++)
	{
		...
		{
			kprintf("		FRAME: %d STATUS: MAPPED PID:%d\n",i,frm_tab[i].fr_pid);
		}
		else
		{
			kprintf("		FRAME: %d STATUS: UNMAPPED PID:%d\n",i,frm_tab[i].fr_pid);
		}
	}
}
...
void test_func7()
{
  ...
  {  
    *((int *)addrs[i]) = i + 1;
	...
	// *((int *)addrs[0]) = 200;
	//print_accessbit(addrs[0]);
	//*((int *)addrs[1]) = 200;
	//print_accessbit(addrs[1]);
	//*((int *)addrs[2]) = 200;
	//*((int *)addrs[3]) = 200;
