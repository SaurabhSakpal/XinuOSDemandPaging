...
struct fr_map_t frm_tab[FREEPAGE];
struct fifo* fifo_head;
struct fifo* fifo_tail;
int ref_couter;
struct bs_map_t bsm_tab[MAX_ID];
...
/*
	Initialize everything related to paging
*/
void initializeEverythingRelatedToPaging()
{
	//kprintf("Iniitialize Everything related to Paging\n");
	init_bsm(); /*Initialize BS data structure*/
	
	init_frm();	/* initialize frames */
	
	init_commonpages();	/*initialize paging tables */

	//itialize page directory
	int avail = 0;
	int *ptr_avail = &avail;
	get_frm(ptr_avail);
	init_pageDirectory(avail,0);
	
	//initalize pdbr
	proctab[0].pdbr = (avail+1024)*4096;	
	//initialize cr3
	unsigned long pdbr = proctab[0].pdbr;
	write_cr3(pdbr);
...
nulluser()				/* babysit CPU when no one is home */
{
	...
	initializeEverythingRelatedToPaging();
	enable_paging();
...
sysinit()
{
	...
	if (maxaddr+1 > HOLESTART)
	{
		...
		mptr->mlen = (int) truncew(((unsigned) HOLESTART -(unsigned)&end));
        	mptr->mlen -= 4;
		kprintf("HOLEEND: %ul MLEN: %ul END: %ul ",HOLEEND,mptr->mlen,&end);
